...
<div  bind:this={cursor} class={`absolute z-[999] pointer-events-none ${showCursor ? 'block' : 'hidden'}`}>
<div class={`rounded-md h-5 w-5 -translate-y-1/2 -translate-x-1/2  origin-center duration-200 backdrop-invert ${mouseDownState && mouseDownClass} ${mouseInLink && mouseHoverClass}`}></div>
</div>
